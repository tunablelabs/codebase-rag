...
class JavaScriptChunker:
    """JavaScript-specific code chunker using tree-sitter"""
    ...
    def create_chunks(self, code: str, file_path: str) -> List[ChunkInfo]:
        """Create chunks from JavaScript code"""
        try:
            chunks = []
            ...
            if not tree:
                raise ValueError("Failed to parse JavaScript code")
    ...
    def _determine_chunk_type(self, node: Node) -> str:
        """Determine the type of JavaScript chunk"""
        type_mapping = {
    ...
    def _enrich_chunks(self, chunks: List[ChunkInfo], root_node: Node, code: str) -> None:
        ...
        try:
            ...
            name_to_chunk = {}
            for chunk in chunks:
                if chunk.type in ['function', 'class', 'method']:
                    # Extract name from first line
                    first_line = chunk.content.splitlines()[0]
                    words = first_line.split()
                    for word in words:
                        if word not in ['function', 'class', 'async', 'export', 'default']:
