...
class RepoParser:
    ...
    async def analyze_repository(self) -> Dict:
        ...
        #             content = f.read()
        #         summary = self.summarizer(content, max_length=50, min_length=20)[0]['summary_text']
        #         doc_summaries[str(doc_file.relative_to(self.repo_path))] = summary
        #     except Exception as e:
        #         self.logger.error(f"Error summarizing {doc_file}: {e}")
        ...
        return {
            ...
            'documentation': {
                ...
                # 'summaries': doc_summaries if doc_summaries else {'status': 'No documentation found'}
            }
        }