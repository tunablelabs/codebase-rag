...
def setup_query_engine(github_url, systm_prompt, ast_bool, forceReindex):
    ...
    try:
        ...
        if forceReindex:
            # Delete the existing collection if forced reindex is true
            try:
                print(f"Force reindexing enabled. Deleting collection: {collection_name}")
        ...
        # Set up vector store after collection has been created or confirmed
...
if __name__ == "__main__":
    try:
        ...
        ast_bool = sys.argv[4]
        forceReindex = sys.argv[5]
        ast_bool = True if ast_bool == 'true' else False
        forceReindex = True if forceReindex == 'true' else False
        # print('ast bool enabled',ast_bool)
        # print('Force Reindex enabled', forceReindex)

        query_engine, repo_ast = setup_query_engine(github_url, system_prompt, ast_bool, forceReindex)
        if query_engine == 'no_files':
            output_json({
                            "response": "This github repo doesn't contain python or javascript files, as of now we are only supporting Python and Javascript"})
        else:
            if query_engine:
                if ast_bool:
                    query = (
