...
def test_code_chunker_happy_path():
    ...
    chunker = rag_v2.chunker.CodeFileChunker(max_tokens=100)

    file_path = os.path.join(os.path.dirname(__file__), "../rag/chunker.py")
    with open(file_path, "r") as file:
        content = file.read()
    metadata = {"file_path": file_path}
    chunks = chunker.chunk(content, metadata)
...
def test_ipynb_chunker_happy_path():
    ...
    code_chunker = rag_v2.chunker.CodeFileChunker(max_tokens=100)
    chunker = rag_v2.chunker.IpynbFileChunker(code_chunker)
