...
from rag_v2.data_manager import GitHubRepoManager
...
class TestGitHubRepoManager(unittest.TestCase):
    ...
    def test_download_clone_success(self, mock_clone):
        ...
        repo_manager = GitHubRepoManager(repo_id="Storia-AI/rag", local_dir="/tmp/test_repo")
        mock_clone.return_value = MagicMock()
        result = repo_manager.download()
        mock_clone.assert_called_once_with(
            "https://github.com/Storia-AI/rag.git", "/tmp/test_repo/Storia-AI/rag", depth=1, single_branch=True
        )
        self.assertTrue(result)

    @patch("rag.data_manager.requests.get")
    def test_is_public_repository(self, mock_get):
        """Test the is_public property to check if a repository is public."""
        mock_get.return_value.status_code = 200
        repo_manager = GitHubRepoManager(repo_id="Storia-AI/rag")
        self.assertTrue(repo_manager.is_public)
        mock_get.assert_called_once_with("https://api.github.com/repos/Storia-AI/rag", timeout=10)
    ...
    def test_default_branch(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            "https://api.github.com/repos/Storia-AI/rag", headers={"Accept": "application/vnd.github.v3+json"}
        )
    ...
    def test_parse_filter_file(self, mock_file):
        ...
        repo_manager = GitHubRepoManager(repo_id="Storia-AI/rag", inclusion_file="dummy_path")
